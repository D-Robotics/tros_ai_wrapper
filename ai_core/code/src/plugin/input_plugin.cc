...
void InputProducerPlugin::Run() {
  while (data_iterator_->HasNext() && !stop_) {
    ...
    VLOG(EXAMPLE_DEBUG) << "Run input msg, id:" << produced_count_;
    produced_count_++;
    WorkflowPlugin::GetInstance()->FeedWorkflow(image_tensor);
  }
  VLOG(EXAMPLE_DEBUG) << "InputProducerPlugin Run finish.";
}

int InputProducerPlugin::Release(ImageTensorPtr msg) {
  data_iterator_->Release(msg.get());
