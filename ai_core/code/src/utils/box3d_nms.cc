...
template <typename T>
int convex_hull_graham(const Point_box3d_nms<T> (&p)[24], const int &num_in,
...
bool cmp_box3d_nms(float x, float y) { return x > y; }

// =>nms_rotated_cpu=>nms_rotated_cpu_kernel
void nms_rotated(std::vector<int> &keep, std::vector<std::vector<float>> &dets,
                 std::vector<float> &scores, float iou_threshold) {
  // dets.numel() == 0 is not supported
  std::vector<int> order_t;
  std::unordered_map<float, int> order_record;
  for (int i = 0; i < scores.size(); i++) {
    order_record[scores[i]] = i;
  }
  std::vector<float> tmp_scores{scores};
  sort(tmp_scores.begin(), tmp_scores.end(), cmp_box3d_nms);
  for (int i = 0; i < tmp_scores.size(); i++) {
    order_t.push_back(order_record[tmp_scores[i]]);
  }
...
void nms_bev(std::vector<int> &keep_res, std::vector<std::vector<float>> &boxes,
             std::vector<float> &scores, float thresh) {
  assert(boxes[0].size() == 5);
  ...
  sort(tmp_scores.begin(), tmp_scores.end(), cmp_box3d_nms);
  for (int i = 0; i < tmp_scores.size(); i++) {
    int tmp_num = order_record[tmp_scores[i]];
    order.push_back(tmp_num);
    boxes_order_res.push_back(boxes[tmp_num]);
    scores_order_res.push_back(scores[tmp_num]);
  }
  ...
  std::vector<int> keep(boxes_res.size());
  nms_rotated(keep, boxes_res, scores_order_res, thresh);
...
void box3d_multiclass_nms(std::vector<std::vector<float>> &bboxes_res,
  ...
  // cfg.use_rotate_nms = true  mlvl_bboxes2d=None
  int num_classes = mlvl_scores[0].size() - 1;
  ...
  for (int i = 0; i < num_classes; i++) {
    ...
    std::vector<bool> cls_inds;
    bool record_any = false;
    for (int mlvl_scores_row = 0; mlvl_scores_row < mlvl_scores.size();
         mlvl_scores_row++) {
      bool tmp = mlvl_scores[mlvl_scores_row][i] > score_thr ? true : false;
      if (tmp) record_any = true;
      cls_inds.push_back(tmp);
    }
    if (!record_any) continue;
    std::vector<float> _scores;
    std::vector<std::vector<float>> _bboxes_for_nms;
    std::vector<std::vector<float>> _mlvl_bboxes;

    for (int tmp_i = 0; tmp_i < mlvl_scores.size(); tmp_i++) {
      if (cls_inds[tmp_i]) {
        _scores.push_back(mlvl_scores[tmp_i][i]);
        _bboxes_for_nms.push_back(mlvl_bboxes_for_nms[tmp_i]);
        _mlvl_bboxes.push_back(mlvl_bboxes[tmp_i]);
      }
    }
    std::vector<int> selected;
    nms_bev(selected, _bboxes_for_nms, _scores, nms_thr);

    std::vector<int> _mlvl_dir_scores;
    std::vector<int> _mlvl_attr_scores;
    for (int tmp = 0; tmp < cls_inds.size(); tmp++) {
      if (cls_inds[tmp]) {
        _mlvl_dir_scores.push_back(mlvl_dir_scores[tmp]);
        _mlvl_attr_scores.push_back(mlvl_attr_scores[tmp]);
      }
    }

    for (int selected_row = 0; selected_row < selected.size(); selected_row++) {
      bboxes.push_back(_mlvl_bboxes[selected[selected_row]]);
      scores.push_back(_scores[selected[selected_row]]);
      dir_scores.push_back(_mlvl_dir_scores[selected[selected_row]]);
      attr_scores.push_back(_mlvl_attr_scores[selected[selected_row]]);
    }

    // std::vector<int> clas_label(selected.size(), i);
    for (int tmp = 0; tmp < selected.size(); tmp++) {
      labels.push_back(i);
    }
  }
  // bboxes.empty() is not supported in cpp, return directily

  if (!bboxes.empty()) {
    if (bboxes.size() > max_num) {
      std::vector<int> inds;

      std::unordered_map<float, int> inds_record;
      for (int tmp_i = 0; tmp_i < scores.size(); tmp_i++) {
        inds_record[scores[tmp_i]] = tmp_i;
      }
      std::vector<float> tmp_scores{scores};
      sort(tmp_scores.begin(), tmp_scores.end(), cmp_box3d_nms);
      /// inds = inds[:max_num]
      for (int tmp_i = 0; tmp_i < max_num; tmp_i++) {
        inds.push_back(inds_record[tmp_scores[tmp_i]]);
      }

      for (auto &tmp : inds) {
        bboxes_res.push_back(bboxes[tmp]);
        labels_res.push_back(labels[tmp]);
        scores_res.push_back(scores[tmp]);
        dir_scores_res.push_back(dir_scores[tmp]);
        attr_scores_res.push_back(attr_scores[tmp]);
      }
    } else {
      bboxes_res = bboxes;
      labels_res = labels;
      scores_res = scores;
      dir_scores_res = dir_scores;
      attr_scores_res = attr_scores;
    }
  } else {
    return;
  }
