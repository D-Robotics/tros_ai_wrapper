...
#include "input/batch_temporal_data_iterator.h"
...
DEFINE_AND_REGISTER_DATA_ITERATOR(batch_temporal_data,
                                  BatchTemporalDataIterator)
...
int BatchTemporalDataIterator::Init(std::string config_string) {
  int ret_code = DataIterator::Init(config_string);
  if (ret_code != 0) {
    return -1;
  }
  temporal_model_ =
      WorkflowPlugin::GetInstance()->Is_temporal_multiframe_model();
...
bool BatchTemporalDataIterator::Next(ImageTensor *image_tensor) {
  ...
  if (cache_able_) {
    ...
    *image_tensor = cache_[send_index_++];
    ...
    std::size_t pos = 0;
    if ((pos = input_name.rfind("-")) != std::string::npos) {
    ...
    for (int i = 1; i < data_files_.size(); i++) {
      ...
      if ((pos = tmp_input_name.rfind("-")) != std::string::npos) {
    ...
    for (size_t cnt{0}; cnt < input_count_; cnt++) {
      if (RelPos_idx_.count(cnt) > 0) {
        if (cnt == 17 || cnt == 18 || cnt == 19) {
          prepare_batch_temporal_mask(&(image_tensor->tensors[cnt]),
                                      data_files_[cnt][send_index_],
                                      cycle_idx_);
        } else {
          prepare_batch_RelPos_and_quanti(&(image_tensor->tensors[cnt]),
                                          data_files_[cnt][send_index_],
                                          cycle_idx_);
        }
      } else {
        if (temporal_model_ && is_temporal_[cnt]) {
          ...
          if (need_quant_) {
            ...
            VLOG(EXAMPLE_SYSTEM)
                << "temporal model input must need_quant like QCNet";
          }
        } else {
          if (need_quant_) {
            prepare_batch_tensor_and_quanti(&(image_tensor->tensors[cnt]),
  ...
  VLOG(EXAMPLE_DETAIL) << " cycle_idx_ " << cycle_idx_;
  image_tensor->frames_per_sample_infer = cycle_idx_;
  // Read the next sample after the last cycle_idx_
  if (cycle_idx_ == infer_fram_nums_ - 1) {
    cycle_idx_ = 0;
    send_index_++;
...
void BatchTemporalDataIterator::Release(ImageTensor *image_tensor) {
  if (!cache_able_) {
    for (int i = 0; i < input_count_; i++) {
...
int BatchTemporalDataIterator::LoadConfig(std::string &config_string) {
...
BatchTemporalDataIterator::~BatchTemporalDataIterator() {}

bool BatchTemporalDataIterator::HasNext() { return !is_finish_; }
