...
#include "input/releated_image_list_data_iterator.h"
...
DEFINE_AND_REGISTER_DATA_ITERATOR(releatedimage, ReleatedImageListDataIterator)
...
int ReleatedImageListDataIterator::Init(std::string config_string) {
  ...
  if (cache_able_) {
    for (int i = 0; i < max_cache_size_; i++) {
      ...
      // all need_update_tenor_num_ + 1 input, 1 is image
      ...
      for (int i = 0; i < need_update_tensor_num_; i++) {
        hbUCPMallocCached(
            &(image_tensor.tensors[i + 2].sysMem),
            image_tensor.tensors[i + 2].properties.alignedByteSize, 0);
        memset(image_tensor.tensors[i + 2].sysMem.virAddr, 0,
               image_tensor.tensors[i + 2].properties.alignedByteSize);
        flush_tensor(&(image_tensor.tensors[i + 2]));
      }
...
bool ReleatedImageListDataIterator::Next(ImageTensor *image_tensor) {
  ...
  if (cache_able_) {
    ...
    *image_tensor = cache_[send_index_++];
    ...
    // all 0
    for (int i = 0; i < need_update_tensor_num_; i++) {
      hbUCPMallocCached(&(image_tensor->tensors[i + 2].sysMem),
                        image_tensor->tensors[i + 2].properties.alignedByteSize,
                        0);
      memset(image_tensor->tensors[i + 2].sysMem.virAddr, 0,
             image_tensor->tensors[i + 2].properties.alignedByteSize);
...
void ReleatedImageListDataIterator::Release(ImageTensor *image_tensor) {
  if (!cache_able_) {
    release_multi_tensor(image_tensor->tensors);
  }
}

int ReleatedImageListDataIterator::LoadConfig(std::string &config_string) {
  ...
  if (document.HasMember("need_update_tensor_num")) {
    need_update_tensor_num_ = document["need_update_tensor_num"].GetInt();
  }
...
ReleatedImageListDataIterator::~ReleatedImageListDataIterator() {}

bool ReleatedImageListDataIterator::HasNext() { return !is_finish_; }
