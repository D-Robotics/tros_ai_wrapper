...
#********************
# 二、默认参数定义（与原脚本一致，仅后续安装路径适配 s600）
type="qat"
dsp="OFF"
image="OFF"
system="linux"
...
function build_dsp_sample() {
    ...
    # 进入 DSP 示例代码目录
    cd ../../../custom_operator/dsp_sample/dsp_code || exit 1
    # 执行 DSP 编译脚本
    bash build_dsp.sh  -a aarch64 || exit 1
    cd -

    centerpoint_path=../s600/qat/script/detection/centerpoint_pointpillar_nuscenes
...
# Linux ARM64 编译函数（核心修改：安装路径从 s100 改为 s600）
function build_arm() {
    echo "===== Build for Linux ARM64 (s600) ====="
    ...
    cmake -DDSP_ON=${dsp} -DCMAKE_INSTALL_PREFIX=../../s600/${type}/script ..
    make -j2
    make install
...
function build_qnx() {
  ...
  cmake -DDSP_ON=${dsp} -DCMAKE_INSTALL_PREFIX=../../s600/${type}/script -DCMAKE_SYSTEM_NAME=QNX ..
