...
#ifndef DNN_AI_BENCHMARK_CODE_INCLUDE_UTILS_POOL_H_
#define DNN_AI_BENCHMARK_CODE_INCLUDE_UTILS_POOL_H_
...
class Pool {
 public:
  ...
  static std::shared_ptr<Pool<T>> Create(int pre_alloc_cnt, int max_alloc_cnt,
                                         Args &&... args) {
  ...
  T *Get(int timeout) {
    ...
    if (timeout > 0) {
      cv_.wait_for(lck, std::chrono::milliseconds(timeout),
                   [this] { return !this->free_list_.empty(); });
    } else {
...
#endif  // DNN_AI_BENCHMARK_CODE_INCLUDE_UTILS_POOL_H_
